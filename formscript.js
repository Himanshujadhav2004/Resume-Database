...
firebase.auth().onAuthStateChanged((user)=>{
    if(!user){
        ...
        var nameInput = document.getElementById('First-Name-input')
        let FirstnameT=document.getElementById("FirstnameT");
        database.ref('users/' + user.uid +'/template1'+'/firstname').once('value')
        .then((snapshot) => {
      ...
      database.ref('users/' + user.uid +'/template1'+'/lastname').once('value')
      .then((snapshot) => {
       ...
       database.ref('users/' + user.uid +'/template1'+'/profession').once('value')
.then((snapshot) => {
 ...
 database.ref('users/' + user.uid +'/template1'+'/city').once('value')
 .then((snapshot) => {
    const city = snapshot.val();
 
    CityInput.value=city
  })
  database.ref('users/' + user.uid +'/template1'+'/country').once('value')
  .then((snapshot) => {
     const city = snapshot.val();
  
     CountryInput.value=city
   })
   database.ref('users/' + user.uid +'/template1'+'/pincode').once('value')
   .then((snapshot) => {
      const city = snapshot.val();
   
      PinCode2Input.value=city
    })
    database.ref('users/'+user.uid+'/template1'+'/phone').once('value')
    .then((snapshot) => {
      const phone = snapshot.val();
   
      PhoneIN.value=phone
    })
    database.ref('users/'+user.uid+'/template1'+'/emailT').once('value')
    .then((snapshot) => {
...
database.ref('users/'+user.uid+'/template1').child('email').set(user.email);
database.ref('users/'+user.uid+'/template1').child('firstname').set(nameInput.value);
database.ref('users/' + user.uid +'/template1'+'/firstname').once('value')
.then((snapshot) => {
   const Firstname = snapshot.val();
...
database.ref('users/'+user.uid+'/template1').child('lastname').set(lastnameName.value);
database.ref('users/' + user.uid +'/template1'+'/lastname').once('value')
.then((snapshot) => {
   const lastname = snapshot.val();
   lastnameT.innerHTML=lastname;
...
database.ref('users/'+user.uid+'/template1').child('profession').set(ProfessionInput.value);
database.ref('users/' + user.uid +'/template1'+'/profession').once('value')
.then((snapshot) => {
   const Profession = snapshot.val();
   ProfessionT.innerHTML=Profession; 
 ...
 database.ref('users/'+user.uid+'/template1').child('city').set(CityInput.value);




 database.ref('users/'+user.uid+'/template1').child('country').set(CountryInput.value);


 database.ref('users/'+user.uid+'/template1').child('pincode').set(PinCode2Input.value)
 database.ref('users/'+user.uid+'/template1').child('phone').set(PhoneIN.value)
 database.ref('users/'+user.uid+'/template1').child('emailT').set(EmailIN.value)
 addrees();
 phoneandemail();
  ...
  function phoneandemail(){
    const database = firebase.database();
var user = firebase.auth().currentUser;
    database.ref('users/' + user.uid + '/template1').once('value')
      .then((snapshot) => {
